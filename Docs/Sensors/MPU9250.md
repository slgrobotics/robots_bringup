...
Use calibration [script](https://github.com/slgrobotics/mpu9250/blob/main/tests/calibrate_mag.py) and set related parameters in the launch file.
...
You can launch the driver in *"raw_only"* or *"full Madgwick filter fusing"* mode. See related launch files [examples](https://github.com/slgrobotics/mpu9250/tree/main/launch).
...
**Note:** Magnetometer calibration is **NOT OPTIONAL**. You must do this.
- Run the [calibration script](https://github.com/slgrobotics/mpu9250/blob/main/tests/calibrate_mag.py) and rotate the sensor through all possible orientations.
- Copy and paste the script’s output into your launch file.

Run the ROS 2 driver using your robot’s launch file, or one of the example launch files [here](https://github.com/slgrobotics/mpu9250/tree/main/launch) (edit them to include your calibrated values).

RViz2 should show the IMU frame oriented correctly:
- When the robot is facing East, the red (x, forward) axis should be parallel to the map frame’s red (x, East) axis.
- The green (y, left) axis should be parallel to the map frame’s green (y, North) axis.
- The blue (z, up) axis should be pointing upward.
- Tilting the robot forward, backward, or sideways should be reflected accurately in the RViz2 display.

If debug printing is enabled (*"verbose": True*), orientation will be printed in the “aircraft” frame (North-East-Down, NED):
- nose up → positive *pitch*
- right wing down → positive *roll*
- yawing right → positive *yaw* (yaw=0 when robot is facing North)

Verify the published */imu/mag* values for your [location](https://www.ngdc.noaa.gov/geomag/calculators/magcalc.shtml?#igrfwmm):
...
Note: The magnetic field is published in *Tesla*, the NOAA website reports values in *microtesla (µT)*.

Rotate the robot in place. The published values should roughly conform to the following matrix:
```
         |   x   |   y   |   z   |
----------------------------------
  North  |  20   |   0   |  -40  |
